...
{
    private const long OpenAiImageLimitBytes = 20 * 1024 * 1024;
    private const double SafetyMargin = 0.95;
    private static readonly long TargetRawBytes = (long)(OpenAiImageLimitBytes * SafetyMargin * 0.75);

    private static readonly int[] MaxSides = [3600, 3200, 2800, 2400, 2000, 1600];
    private static readonly int[] Qualities = [90, 84, 78, 70];
    ...
    public static LoadResult Load(string path, CancellationToken cancellationToken = default)
    {
            ...
            foreach (var quality in Qualities)
            {
                ...
                if (data.LongLength <= TargetRawBytes)
                {
                    var note = $"Original {raw.Length:N0} bytes resized to {working.Width}x{working.Height} " +
                               $"JPEG q={quality} ({data.Length:N0} bytes) to fit OpenAI's 20MB image limit.";
        ...
        throw new InvalidOperationException(
            $"Could not shrink {Path.GetFileName(path)} below {TargetRawBytes:N0} bytes even at 1600px / q=70.");
    }
